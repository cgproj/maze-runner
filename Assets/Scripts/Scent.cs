...
public struct Scent
{
    ...
    {
        scentA = new NativeArray<float>(maze.Length, Allocator.Persistent, NativeArrayOptions.UninitializedMemory);
    ...
    public NativeArray<float> Disperse(Maze maze, Vector3 playerPosition)
    {
        ...
        if (cooldown <= 0f)
        {
            ...
            }.ScheduleParallel(maze.Length, maze.SizeWidth, default).Complete();
            useA = !useA;
        }
